"""Consolidated base for research signals.

Every ``*_signal.py`` clone in this package was byte-identical apart from the
module docstring, the per-signal Observation dataclass name, the Signal class
name, and the ``name`` attribute. They are collapsed here into one Observation
dataclass and one generic Signal class parameterized by ``name`` (carried on a
frozen spec). The method bodies below are copied verbatim from the original
clones (the ``raw_value`` formula is identical across all signals).

Note: this family is an illustrative, uncalibrated deterministic heuristic
that demonstrates the *shape* of a production fund system. Its signal
coefficients and labels are placeholders, not validated models. It is
standalone and not wired into the committee decision (only sampled by a
couple of tests).
"""
...
class BaseSignal:
    ...
    def evaluate(self, prop: PropertyInput, metrics: UnderwritingMetrics, data: DiligenceDataBundle) -> SignalObservation:
