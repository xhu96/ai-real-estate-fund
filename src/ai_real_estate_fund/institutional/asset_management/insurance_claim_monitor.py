...
"""Asset-management monitor for insurance claims.
...
class InsuranceClaimMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'risk_manager'
    enabled: bool = True


class InsuranceClaimMonitor:
    name = 'insurance claims'
    description = 'claim frequency and retained losses'
    owner = 'risk_manager'

    def __init__(self, config: InsuranceClaimMonitorConfig | None = None) -> None:
        self.config = config or InsuranceClaimMonitorConfig()
