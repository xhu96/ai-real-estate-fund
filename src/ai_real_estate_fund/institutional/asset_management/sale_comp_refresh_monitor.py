...
"""Asset-management monitor for sale comp refresh.
...
class SaleCompRefreshMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'research_lead'
    enabled: bool = True


class SaleCompRefreshMonitor:
    name = 'sale comp refresh'
    description = 'freshness of sale comp evidence'
    owner = 'research_lead'

    def __init__(self, config: SaleCompRefreshMonitorConfig | None = None) -> None:
        self.config = config or SaleCompRefreshMonitorConfig()
