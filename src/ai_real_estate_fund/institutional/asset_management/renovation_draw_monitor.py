...
"""Asset-management monitor for renovation draws.
...
class RenovationDrawMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'construction_manager'
    enabled: bool = True


class RenovationDrawMonitor:
    name = 'renovation draws'
    description = 'draws funded versus completion milestones'
    owner = 'construction_manager'

    def __init__(self, config: RenovationDrawMonitorConfig | None = None) -> None:
        self.config = config or RenovationDrawMonitorConfig()
