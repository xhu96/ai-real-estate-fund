...
"""Asset-management monitor for exit readiness.
...
class ExitReadinessMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'portfolio_manager'
    enabled: bool = True


class ExitReadinessMonitor:
    name = 'exit readiness'
    description = 'materials required for sale/refi process'
    owner = 'portfolio_manager'

    def __init__(self, config: ExitReadinessMonitorConfig | None = None) -> None:
        self.config = config or ExitReadinessMonitorConfig()
