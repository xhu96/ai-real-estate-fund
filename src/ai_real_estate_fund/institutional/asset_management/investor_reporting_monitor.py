...
"""Asset-management monitor for investor reporting.
...
class InvestorReportingMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'investor_relations'
    enabled: bool = True


class InvestorReportingMonitor:
    name = 'investor reporting'
    description = 'investor package timeliness and quality'
    owner = 'investor_relations'

    def __init__(self, config: InvestorReportingMonitorConfig | None = None) -> None:
        self.config = config or InvestorReportingMonitorConfig()
