...
"""Asset-management monitor for leasing velocity.
...
class LeasingVelocityMonitorConfig:
    target: float = 1.0
    tolerance: float = 0.10
    unit: str = "ratio"
    owner: str = 'leasing_manager'
    enabled: bool = True


class LeasingVelocityMonitor:
    name = 'leasing velocity'
    description = 'new leases signed per available unit'
    owner = 'leasing_manager'

    def __init__(self, config: LeasingVelocityMonitorConfig | None = None) -> None:
        self.config = config or LeasingVelocityMonitorConfig()
