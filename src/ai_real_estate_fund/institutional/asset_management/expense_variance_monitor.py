...
"""Asset-management monitor for expense variance.
...
class ExpenseVarianceMonitorConfig:
    target: float = 1.0
...
class ExpenseVarianceMonitor:
    name = 'expense variance'
    description = 'actual controllable expenses versus budget'
    owner = 'controller'

    def __init__(self, config: ExpenseVarianceMonitorConfig | None = None) -> None:
        self.config = config or ExpenseVarianceMonitorConfig()
