...
"""Model-validation check for calibration curve."""
...
class CalibrationCurveValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class CalibrationCurveValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> CalibrationCurveValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return CalibrationCurveValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
