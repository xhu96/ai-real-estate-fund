...
"""Model-validation check for memo consistency."""
...
class MemoConsistencyValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class MemoConsistencyValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> MemoConsistencyValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return MemoConsistencyValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
