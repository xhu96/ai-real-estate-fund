...
"""Model-validation check for hard-stop enforcement."""
...
class HardStopEnforcementValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class HardStopEnforcementValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> HardStopEnforcementValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return HardStopEnforcementValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
