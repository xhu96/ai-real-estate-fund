...
"""Model-validation check for agent score drift."""
...
class AgentScoreDriftValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class AgentScoreDriftValidator:
    name = 'agent score drift'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> AgentScoreDriftValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return AgentScoreDriftValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
