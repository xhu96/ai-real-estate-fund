...
"""Model-validation check for capital-stack consistency."""
...
class CapitalStackConsistencyValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class CapitalStackConsistencyValidator:
    name = 'capital-stack consistency'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> CapitalStackConsistencyValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return CapitalStackConsistencyValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
