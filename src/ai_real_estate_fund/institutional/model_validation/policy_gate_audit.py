...
"""Model-validation check for policy gate audit."""
...
class PolicyGateAuditValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class PolicyGateAuditValidator:
    name = 'policy gate audit'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> PolicyGateAuditValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return PolicyGateAuditValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
