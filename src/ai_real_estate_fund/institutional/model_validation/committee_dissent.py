...
"""Model-validation check for committee dissent."""
...
class CommitteeDissentValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class CommitteeDissentValidator:
    name = 'committee dissent'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> CommitteeDissentValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return CommitteeDissentValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
