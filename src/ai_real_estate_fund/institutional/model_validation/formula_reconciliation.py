...
"""Model-validation check for formula reconciliation."""
...
class FormulaReconciliationValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class FormulaReconciliationValidator:
    name = 'formula reconciliation'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> FormulaReconciliationValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return FormulaReconciliationValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
