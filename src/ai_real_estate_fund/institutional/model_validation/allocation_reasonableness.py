...
"""Model-validation check for allocation reasonableness."""
...
class AllocationReasonablenessValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class AllocationReasonablenessValidator:
    name = 'allocation reasonableness'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> AllocationReasonablenessValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return AllocationReasonablenessValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
