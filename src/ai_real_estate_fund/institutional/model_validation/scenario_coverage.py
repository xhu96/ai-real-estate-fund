...
"""Model-validation check for scenario coverage."""
...
class ScenarioCoverageValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class ScenarioCoverageValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> ScenarioCoverageValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return ScenarioCoverageValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
