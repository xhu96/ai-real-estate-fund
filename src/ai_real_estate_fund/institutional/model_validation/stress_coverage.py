...
"""Model-validation check for stress coverage."""
...
class StressCoverageValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class StressCoverageValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> StressCoverageValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return StressCoverageValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
