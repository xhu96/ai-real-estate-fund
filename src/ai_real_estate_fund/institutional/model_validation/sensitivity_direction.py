...
"""Model-validation check for sensitivity direction."""
...
class SensitivityDirectionValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class SensitivityDirectionValidator:
    name = 'sensitivity direction'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> SensitivityDirectionValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return SensitivityDirectionValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
