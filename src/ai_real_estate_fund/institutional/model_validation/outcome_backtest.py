...
"""Model-validation check for outcome backtest."""
...
class OutcomeBacktestValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class OutcomeBacktestValidator:
    name = 'outcome backtest'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> OutcomeBacktestValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return OutcomeBacktestValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
