...
"""Model-validation check for source concentration."""
...
class SourceConcentrationValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class SourceConcentrationValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> SourceConcentrationValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return SourceConcentrationValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
