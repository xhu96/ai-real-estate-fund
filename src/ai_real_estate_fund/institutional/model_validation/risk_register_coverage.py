...
"""Model-validation check for risk-register coverage."""
...
class RiskRegisterCoverageValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class RiskRegisterCoverageValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> RiskRegisterCoverageValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return RiskRegisterCoverageValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
