...
"""Model-validation check for data-room traceability."""
...
class DataRoomTraceabilityValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class DataRoomTraceabilityValidator:
    name = 'data-room traceability'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> DataRoomTraceabilityValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return DataRoomTraceabilityValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
