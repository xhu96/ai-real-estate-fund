...
"""Model-validation check for assumption staleness."""
...
class AssumptionStalenessValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class AssumptionStalenessValidator:
    name = 'assumption staleness'
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> AssumptionStalenessValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return AssumptionStalenessValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
