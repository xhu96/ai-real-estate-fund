...
"""Model-validation check for version-control evidence."""
...
class VersionControlValidatorResult:
    validator: str
    passed: bool
    score: float
    findings: list[str]
...
class VersionControlValidator:
    ...
    def validate(self, decision: DictLikeDecision | dict[str, Any]) -> VersionControlValidatorResult:
        payload = self.extract(decision)
        score = self.score(payload)
        return VersionControlValidatorResult(
            validator=self.name,
            passed=score >= 70.0,
            score=score,
