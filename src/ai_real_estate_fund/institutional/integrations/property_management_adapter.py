...
"""Offline adapter stub for property management.
...
class PropertyManagementAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class PropertyManagementAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class PropertyManagementAdapter:
    provider_name = 'property management'
    expected_fields = ['rent_roll', 'ledger', 'work_orders', 'leases']
    ...
    def plan_query(self, query: PropertyManagementAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: PropertyManagementAdapterQuery) -> list[PropertyManagementAdapterRecord]:
        planned = self.plan_query(query)
        return [
            PropertyManagementAdapterRecord(
    ...
    def normalize(self, records: list[PropertyManagementAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: PropertyManagementAdapterRecord) -> list[str]:
        warnings: list[str] = []
