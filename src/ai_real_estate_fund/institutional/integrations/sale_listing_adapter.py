...
"""Offline adapter stub for sale listings.
...
class SaleListingAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
class SaleListingAdapter:
    provider_name = 'sale listings'
    expected_fields = ['asking_price', 'sale_price', 'close_date', 'days_on_market']
    ...
    def plan_query(self, query: SaleListingAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: SaleListingAdapterQuery) -> list[SaleListingAdapterRecord]:
        planned = self.plan_query(query)
        return [
            SaleListingAdapterRecord(
    ...
    def normalize(self, records: list[SaleListingAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: SaleListingAdapterRecord) -> list[str]:
        warnings: list[str] = []
