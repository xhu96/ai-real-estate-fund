...
"""Offline adapter stub for lender quotes.
...
class LenderQuoteAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
class LenderQuoteAdapter:
    provider_name = 'lender quotes'
    expected_fields = ['interest_rate', 'term', 'amortization', 'fees']
    ...
    def plan_query(self, query: LenderQuoteAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: LenderQuoteAdapterQuery) -> list[LenderQuoteAdapterRecord]:
        planned = self.plan_query(query)
        return [
            LenderQuoteAdapterRecord(
    ...
    def normalize(self, records: list[LenderQuoteAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: LenderQuoteAdapterRecord) -> list[str]:
        warnings: list[str] = []
