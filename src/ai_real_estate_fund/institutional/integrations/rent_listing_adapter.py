...
"""Offline adapter stub for rent listings.
...
class RentListingAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
class RentListingAdapter:
    provider_name = 'rent listings'
    expected_fields = ['asking_rent', 'bedrooms', 'bathrooms', 'days_on_market']
    ...
    def plan_query(self, query: RentListingAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: RentListingAdapterQuery) -> list[RentListingAdapterRecord]:
        planned = self.plan_query(query)
        return [
            RentListingAdapterRecord(
    ...
    def normalize(self, records: list[RentListingAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: RentListingAdapterRecord) -> list[str]:
        warnings: list[str] = []
