...
"""Offline adapter stub for document storage.
...
class DocumentStorageAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class DocumentStorageAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class DocumentStorageAdapter:
    provider_name = 'document storage'
    expected_fields = ['document_id', 'category', 'uploaded_at', 'checksum']
    ...
    def plan_query(self, query: DocumentStorageAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: DocumentStorageAdapterQuery) -> list[DocumentStorageAdapterRecord]:
        planned = self.plan_query(query)
        return [
            DocumentStorageAdapterRecord(
    ...
    def normalize(self, records: list[DocumentStorageAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: DocumentStorageAdapterRecord) -> list[str]:
        warnings: list[str] = []
