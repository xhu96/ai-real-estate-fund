...
"""Offline adapter stub for geocoding.
...
class GeocoderAdapter:
    provider_name = 'geocoding'
    expected_fields = ['latitude', 'longitude', 'confidence', 'match_type']
    ...
    def plan_query(self, query: GeocoderAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: GeocoderAdapterQuery) -> list[GeocoderAdapterRecord]:
        planned = self.plan_query(query)
        return [
            GeocoderAdapterRecord(
    ...
    def normalize(self, records: list[GeocoderAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: GeocoderAdapterRecord) -> list[str]:
        warnings: list[str] = []
