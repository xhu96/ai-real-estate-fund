...
"""Offline adapter stub for contractor bids.
...
class ContractorBidAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class ContractorBidAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class ContractorBidAdapter:
    provider_name = 'contractor bids'
    expected_fields = ['trade', 'scope', 'cost', 'duration']
    ...
    def plan_query(self, query: ContractorBidAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: ContractorBidAdapterQuery) -> list[ContractorBidAdapterRecord]:
        planned = self.plan_query(query)
        return [
            ContractorBidAdapterRecord(
    ...
    def normalize(self, records: list[ContractorBidAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: ContractorBidAdapterRecord) -> list[str]:
        warnings: list[str] = []
