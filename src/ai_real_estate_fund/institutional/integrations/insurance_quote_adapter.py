...
"""Offline adapter stub for insurance quotes.
...
class InsuranceQuoteAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class InsuranceQuoteAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class InsuranceQuoteAdapter:
    provider_name = 'insurance quotes'
    expected_fields = ['premium', 'deductible', 'coverage', 'exclusions']
    ...
    def plan_query(self, query: InsuranceQuoteAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: InsuranceQuoteAdapterQuery) -> list[InsuranceQuoteAdapterRecord]:
        planned = self.plan_query(query)
        return [
            InsuranceQuoteAdapterRecord(
    ...
    def normalize(self, records: list[InsuranceQuoteAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: InsuranceQuoteAdapterRecord) -> list[str]:
        warnings: list[str] = []
