...
"""Offline adapter stub for county recorder.
...
class CountyRecorderAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class CountyRecorderAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class CountyRecorderAdapter:
    ...
    expected_fields = ['deed_date', 'grantor', 'grantee', 'recording_number']
    ...
    def plan_query(self, query: CountyRecorderAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: CountyRecorderAdapterQuery) -> list[CountyRecorderAdapterRecord]:
        planned = self.plan_query(query)
        return [
            CountyRecorderAdapterRecord(
    ...
    def normalize(self, records: list[CountyRecorderAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: CountyRecorderAdapterRecord) -> list[str]:
        warnings: list[str] = []
