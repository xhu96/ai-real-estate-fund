...
"""Offline adapter stub for building permits.
...
class BuildingPermitAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class BuildingPermitAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class BuildingPermitAdapter:
    provider_name = 'building permits'
    expected_fields = ['permit_type', 'units', 'value', 'issued_date']
    ...
    def plan_query(self, query: BuildingPermitAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: BuildingPermitAdapterQuery) -> list[BuildingPermitAdapterRecord]:
        planned = self.plan_query(query)
        return [
            BuildingPermitAdapterRecord(
    ...
    def normalize(self, records: list[BuildingPermitAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: BuildingPermitAdapterRecord) -> list[str]:
        warnings: list[str] = []
