...
"""Offline adapter stub for flood maps.
...
class FloodMapAdapter:
    provider_name = 'flood maps'
    expected_fields = ['fema_zone', 'base_flood_elevation', 'panel', 'effective_date']
    ...
    def plan_query(self, query: FloodMapAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: FloodMapAdapterQuery) -> list[FloodMapAdapterRecord]:
        planned = self.plan_query(query)
        return [
            FloodMapAdapterRecord(
    ...
    def normalize(self, records: list[FloodMapAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: FloodMapAdapterRecord) -> list[str]:
        warnings: list[str] = []
