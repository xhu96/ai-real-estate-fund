"""Generic offline adapter base for real-estate data integrations.
...
Every concrete provider is identical apart from three per-instance
constants (provider_name + expected_fields) and the preserved class
names. Those constants live in an ``AdapterSpec``; the configured
``Adapter``/``AdapterQuery``/``AdapterRecord`` classes are generated from
each spec by :mod:`ai_real_estate_fund.institutional.integrations.specs`.

Note: this family is an illustrative, uncalibrated deterministic heuristic
that demonstrates the *shape* of a production fund system. Its fixture data,
field weights, and provider labels are placeholders, not validated models or
real feeds. It is standalone and not wired into the committee decision (only
sampled by a couple of tests).
"""
...
class Adapter:
    ...
    def plan_query(self, query: AdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: AdapterQuery) -> list[AdapterRecord]:
    ...
    def normalize(self, records: list[AdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: AdapterRecord) -> list[str]:
        warnings: list[str] = []
    ...
    # ``_record_cls`` defaults to the generic record but is overridden on
    # generated subclasses so ``fetch`` builds the provider-named record.
    _record_cls = AdapterRecord
