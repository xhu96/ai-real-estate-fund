...
"""Offline adapter stub for employment data.
...
class EmploymentAdapter:
    provider_name = 'employment data'
    expected_fields = ['jobs', 'unemployment', 'wages', 'industry']
    ...
    def plan_query(self, query: EmploymentAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: EmploymentAdapterQuery) -> list[EmploymentAdapterRecord]:
        planned = self.plan_query(query)
        return [
            EmploymentAdapterRecord(
    ...
    def normalize(self, records: list[EmploymentAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: EmploymentAdapterRecord) -> list[str]:
        warnings: list[str] = []
