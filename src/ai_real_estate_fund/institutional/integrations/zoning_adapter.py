...
"""Offline adapter stub for zoning data.
...
class ZoningAdapter:
    provider_name = 'zoning data'
    expected_fields = ['district', 'use', 'density', 'parking']
    ...
    def plan_query(self, query: ZoningAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: ZoningAdapterQuery) -> list[ZoningAdapterRecord]:
    ...
    def normalize(self, records: list[ZoningAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: ZoningAdapterRecord) -> list[str]:
        warnings: list[str] = []
