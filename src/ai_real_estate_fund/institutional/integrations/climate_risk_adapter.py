...
"""Offline adapter stub for climate risk.
...
class ClimateRiskAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
class ClimateRiskAdapter:
    provider_name = 'climate risk'
    expected_fields = ['flood', 'wildfire', 'wind', 'heat']
    ...
    def plan_query(self, query: ClimateRiskAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: ClimateRiskAdapterQuery) -> list[ClimateRiskAdapterRecord]:
        planned = self.plan_query(query)
        return [
            ClimateRiskAdapterRecord(
    ...
    def normalize(self, records: list[ClimateRiskAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: ClimateRiskAdapterRecord) -> list[str]:
        warnings: list[str] = []
