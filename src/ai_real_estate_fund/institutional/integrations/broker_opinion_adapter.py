...
"""Offline adapter stub for broker opinions.
...
class BrokerOpinionAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class BrokerOpinionAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class BrokerOpinionAdapter:
    provider_name = 'broker opinions'
    expected_fields = ['value_range', 'cap_rate', 'buyer_pool', 'marketing_time']
    ...
    def plan_query(self, query: BrokerOpinionAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: BrokerOpinionAdapterQuery) -> list[BrokerOpinionAdapterRecord]:
        planned = self.plan_query(query)
        return [
            BrokerOpinionAdapterRecord(
    ...
    def normalize(self, records: list[BrokerOpinionAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: BrokerOpinionAdapterRecord) -> list[str]:
        warnings: list[str] = []
