...
"""Offline adapter stub for title commitment.
...
class TitleAdapter:
    provider_name = 'title commitment'
    expected_fields = ['exceptions', 'easements', 'liens', 'vesting']
    ...
    def plan_query(self, query: TitleAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: TitleAdapterQuery) -> list[TitleAdapterRecord]:
    ...
    def normalize(self, records: list[TitleAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: TitleAdapterRecord) -> list[str]:
        warnings: list[str] = []
