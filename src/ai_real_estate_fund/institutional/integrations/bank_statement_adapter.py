...
"""Offline adapter stub for bank statements.
...
class BankStatementAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class BankStatementAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class BankStatementAdapter:
    provider_name = 'bank statements'
    expected_fields = ['deposit', 'withdrawal', 'date', 'counterparty']
    ...
    def plan_query(self, query: BankStatementAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: BankStatementAdapterQuery) -> list[BankStatementAdapterRecord]:
        planned = self.plan_query(query)
        return [
            BankStatementAdapterRecord(
    ...
    def normalize(self, records: list[BankStatementAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: BankStatementAdapterRecord) -> list[str]:
        warnings: list[str] = []
