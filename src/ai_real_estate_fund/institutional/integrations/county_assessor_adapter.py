...
"""Offline adapter stub for county assessor.
...
class CountyAssessorAdapterRecord:
    source: str
    external_id: str
    fields: dict[str, Any]
...
@dataclass(slots=True)
class CountyAssessorAdapterQuery:
    address: str
    market: str = ""
    parcel_id: str = ""
    limit: int = 25
...
class CountyAssessorAdapter:
    provider_name = 'county assessor'
    expected_fields = ['parcel_id', 'assessed_value', 'tax_year', 'owner_name']
    ...
    def plan_query(self, query: CountyAssessorAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: CountyAssessorAdapterQuery) -> list[CountyAssessorAdapterRecord]:
        planned = self.plan_query(query)
        return [
            CountyAssessorAdapterRecord(
    ...
    def normalize(self, records: list[CountyAssessorAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: CountyAssessorAdapterRecord) -> list[str]:
        warnings: list[str] = []
