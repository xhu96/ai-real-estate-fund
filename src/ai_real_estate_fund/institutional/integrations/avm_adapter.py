...
"""Offline adapter stub for automated valuation.
...
class AvmAdapter:
    provider_name = 'automated valuation'
    expected_fields = ['value', 'confidence', 'range_low', 'range_high']
    ...
    def plan_query(self, query: AvmAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: AvmAdapterQuery) -> list[AvmAdapterRecord]:
    ...
    def normalize(self, records: list[AvmAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: AvmAdapterRecord) -> list[str]:
        warnings: list[str] = []
