...
"""Offline adapter stub for census data.
...
class CensusAdapter:
    provider_name = 'census data'
    expected_fields = ['population', 'households', 'income', 'renter_share']
    ...
    def plan_query(self, query: CensusAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: CensusAdapterQuery) -> list[CensusAdapterRecord]:
    ...
    def normalize(self, records: list[CensusAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: CensusAdapterRecord) -> list[str]:
        warnings: list[str] = []
