...
"""Offline adapter stub for accounting system.
...
class AccountingAdapter:
    provider_name = 'accounting system'
    expected_fields = ['trial_balance', 'general_ledger', 'cash_receipts', 'payables']
    ...
    def plan_query(self, query: AccountingAdapterQuery) -> dict[str, Any]:
        return {
    ...
    def fetch(self, query: AccountingAdapterQuery) -> list[AccountingAdapterRecord]:
        planned = self.plan_query(query)
        return [
            AccountingAdapterRecord(
    ...
    def normalize(self, records: list[AccountingAdapterRecord]) -> list[dict[str, Any]]:
    ...
    def quality_warnings(self, record: AccountingAdapterRecord) -> list[str]:
        warnings: list[str] = []
