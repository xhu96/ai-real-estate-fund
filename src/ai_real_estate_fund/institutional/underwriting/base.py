"""Shared base for the underwriting model family.

The 25 ``*_model.py`` files were byte-identical clones differing only in
their class name plus two string constants (``subject`` and ``lens``). This
module collapses them into one generic ``Model`` plus a frozen ``ModelSpec``
holding the per-instance constants. Concrete classes preserving the original
names are generated from the spec list in :mod:`.specs`.

The model intentionally stays dependency-free so the repository can run in
educational or offline environments. Production deployments would swap these
deterministic heuristics for source-verified statements, third-party market
data, and reviewed Excel/Python model outputs.

Note: this family is an illustrative, uncalibrated deterministic heuristic
that demonstrates the *shape* of a production fund system. Its scoring
coefficients and subject/lens labels (e.g. the "equity waterfall" subject)
are placeholders, not validated models. It is standalone and not wired into
the committee decision (only sampled by a couple of tests).
"""
...
class ModelSpec:
    """Per-instance constants that distinguish one underwriting model."""
...
class Model:
    """Generic underwriting model; ``subject``/``lens`` come from a spec."""
    ...
    def evaluate(self, prop: PropertyInput, metrics: UnderwritingMetrics) -> ModelResult:
        return ModelResult(
