...
class PropertyManagementAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        positives, concerns = [], []
        if 0.06 <= prop.management_rate <= 0.10:
            positives.append(f"Management fee of {pct(prop.management_rate)} is in a plausible range.")
        else:
            concerns.append(f"Management fee of {pct(prop.management_rate)} may be unrealistic for this asset type.")
        if prop.unit_count > 4:
            concerns.append("Larger asset requires stronger property-management systems than a small rental.")
