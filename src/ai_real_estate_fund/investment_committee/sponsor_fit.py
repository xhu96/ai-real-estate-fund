...
class SponsorFitAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        if prop.unit_count >= 4:
            complexity += 12
        if prop.rehab_budget / prop.purchase_price > 0.15:
            complexity += 18
        ...
        positives, concerns = [], []
        if complexity <= 12:
            positives.append("Deal complexity appears manageable for a small-operator playbook.")
        else:
            concerns.append("Deal complexity requires experienced property management, contractor control, and lender communication.")
