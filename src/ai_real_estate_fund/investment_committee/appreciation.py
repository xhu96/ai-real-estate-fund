...
class AppreciationAssumptionAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        positives, concerns = [], []
        if abs(spread) <= 0.01: positives.append("Appreciation assumption is close to market snapshot.")
        else: concerns.append(f"Appreciation assumption differs from market snapshot by {pct(spread)}.")
        if prop.estimated_arv and arv_premium > 0.15: positives.append(f"Estimated ARV implies {pct(arv_premium)} value uplift over purchase price.")
