...
class PropertyConditionAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        positives, concerns = [], []
        if age <= 35: positives.append("Asset age is not a major negative in the default screen.")
        elif age >= 75: concerns.append("Asset age raises major-systems and environmental diligence requirements.")
        if rehab_ratio > 0.20: concerns.append(f"Rehab ratio of {pct(rehab_ratio)} suggests meaningful deferred-maintenance risk.")
