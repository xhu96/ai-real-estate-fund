...
class NeighborhoodQualityAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        positives, concerns = [], []
        if prop.neighborhood_score >= 7:
            positives.append("Neighborhood score supports tenant demand and exit liquidity.")
        else:
            concerns.append("Neighborhood score is not strong enough to rely on broad market averages.")
        if prop.crime_risk_score >= 7:
            concerns.append("Crime-risk score requires street-by-street validation.")
        if prop.school_score >= 7:
            positives.append("School score is a positive demand signal for family rentals.")
