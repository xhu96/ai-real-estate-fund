...
class LiquidityStressAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        score = score_range(dscr if dscr != float("inf") else 2.0, 0.85, 1.35) * 0.65 + score_range(coc, -0.08, 0.08) * 0.35
        positives, concerns = [], []
        if dscr >= 1.10: positives.append(f"Stressed DSCR remains above 1.10x at {dscr:.2f}x.")
        else: concerns.append(f"Stressed DSCR falls to {dscr:.2f}x.")
        if coc < 0: concerns.append(f"Stressed cash-on-cash return turns negative at {pct(coc)}.")
        return self.finding(score=score, confidence=0.78, thesis=f"Stress case cuts rent 7%, raises controllable expenses 12%, and adds 125 bps to debt cost.", positives=positives, concerns=concerns, evidence=[self.metric("stress_dscr", round(dscr,3)), self.metric("stress_cash_on_cash", round(coc,4))], actions=["Keep liquidity reserve sized to survive the stress case without forced sale or covenant breach."])
