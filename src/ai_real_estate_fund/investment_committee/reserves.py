...
class ReservePlanningAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        positives, concerns = [], []
        if reserve_ratio >= 0.06: positives.append(f"Repairs/capex reserves equal {pct(reserve_ratio)} of GPI.")
        else: concerns.append(f"Repairs/capex reserves equal only {pct(reserve_ratio)} of GPI.")
