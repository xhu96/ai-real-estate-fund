...
class ExpenseLoadAgent(CommitteeAgent):
    ...
    def analyze(self, prop, metrics, data, prior_findings):
        ...
        if expense_ratio <= 0.45:
            positives.append(f"Expense ratio of {pct(expense_ratio)} is within a normal rental range.")
        else:
            concerns.append(f"Expense ratio of {pct(expense_ratio)} is high and compresses NOI.")
        if prop.capex_annual <= 0:
            concerns.append("Capex reserve is zero; this is rarely appropriate for production underwriting.")
